...
/**
 * Distinguishes marks that share a name.
 *
 * Where a client has several concepts in the set they are separate pieces of
 * work on separate pages, but they were all being given the same title and the
 * same description - three Raven Crux pages and three Joey Taylor pages each
 * competing with themselves in search for a query only one of them can answer.
 *
 * Where the concepts are genuinely different forms the form is the honest
 * distinction and it is used. Where they are the same form there is nothing
 * true to say beyond which one it is, so they are numbered.
 */
function markGroup(item) {
  const key = item.name || `${item.category} concept`
  const group = portfolioLogos.filter((l) => (l.name || `${l.category} concept`) === key)
  if (group.length < 2) return null

  const position = group.indexOf(item) + 1
  const forms = group.map((l) => detectForm(l).name)
  const formIsDistinct = forms.filter((f) => f === detectForm(item).name).length === 1

  return {
    position,
    total: group.length,
    qualifier: formIsDistinct ? detectForm(item).name.toLowerCase() : `concept ${position}`,
  }
}
...
export const caseStudies = portfolioLogos.map((item, index) => {
  ...
  const group = markGroup(item)

  const siblings = item.name
    ? portfolioLogos.filter((l) => l.name === item.name && l.slug !== item.slug).map((l) => l.slug)
    : []

  /** " - Emblem" / " - Concept 2", or nothing at all for a mark that stands alone. */
  const suffix = group ? ` - ${group.qualifier.charAt(0).toUpperCase()}${group.qualifier.slice(1)}` : ''
  ...
  return {
    ...
    index,
    siblings,
    /* Layout appends " | LogoOrbit", so everything here has to fit inside
       about fifty characters or a search result prints an ellipsis instead of
       the end of it. A long client name drops the category rather than being
       cut off mid-word - the category is on the page and in the description
       either way, and the name is the part somebody searched for. */
    metaTitle: item.name
      ? `${item.name} Logo Design${suffix}${
          `${item.name} Logo Design${suffix}`.length > 34 ? '' : ` - ${item.category}`
        }`
      : `${item.category} Logo Concept${suffix} - Portfolio`,
    /* Kept inside what a search result will actually print. The grouped
       variant says which concept it is up front rather than appending the
       qualifier to the end of an already-full sentence, which pushed these
       past two hundred characters and got the useful half truncated away. */
    metaDescription: item.name
      ? group
        ? `The ${group.qualifier} drawn for ${item.name}, one of ${group.total} ${item.category.toLowerCase()} concepts by the LogoOrbit studio: the brief it answers, where it works, and what was delivered.`
      ...
      : `An original ${item.category.toLowerCase()} logo concept from the LogoOrbit studio${
          group ? ` (${group.qualifier})` : ''
        }, shown as drawing work: how it was built and what it had to survive.`,
    intro: item.name
