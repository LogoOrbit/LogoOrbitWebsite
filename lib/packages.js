...
/**
 * The one-line description for a package page.
 *
 * Names and section titles overlap across the catalogue - "The Complete Brand"
 * sits in a section called "Complete brand package", "Platinum" sits in "Logo
 * Design" - so the noun is chosen rather than concatenated, and the whole
 * thing is trimmed to a length a search result will actually print.
 */
function describePackage(pkg, sectionTitle, priceLabel, lead) {
  const name = pkg.name.toLowerCase()
  const section = sectionTitle.toLowerCase()
  const saidAlready = name.includes(section.replace(/\s*packages?$/, ''))

  const noun = saidAlready ? '' : ` ${section}${/packages?$/.test(section) ? '' : ' package'}`
  const opening = `${pkg.name}${noun} from LogoOrbit at ${priceLabel}.`
  const full = `${opening} ${lead}`

  if (full.length <= 158) return full
  if (opening.length >= 155) return opening
  return `${full.slice(0, 157).replace(/[\s,;:.-]+\S*$/, '')}…`
}

/**
 * Almost everything we sell arrives as a file. The handful of packages that
 * include business cards we print and put in the post are the exception, and
 * the structured data has to say so - a delivery estimate that claims a
 * courier can do same-day worldwide is wrong in a way Google now checks.
 * Detected from the copy rather than hand-flagged, so a new package that
 * promises print gets the honest shipping estimate without anyone remembering
 * to tick a box.
 */
const PRINT_AND_POST = /print(?:ed)? and post/i
const shipsPrint = (source) => PRINT_AND_POST.test(JSON.stringify(source))
...
function buildPackage({ pkg, slug, family, sectionTitle, sectionId, siblings, kind }) {
  ...
  return {
    ...
    /* The section title is dropped when the package name has already said it,
       and the word "package" only added when neither has. Unconditional, this
       produced "The Complete Brand complete brand package package from
       LogoOrbit at $1,899" - the flagship's own page describing itself twice
       and then a third time.

       The old cap was 300 characters, which is roughly twice what a search
       result prints, so the sentence that mattered was always the one cut
       off. It now trims to a word boundary inside the printable length. */
    metaDescription: describePackage(pkg, sectionTitle, priceLabel, pkg.bestFor || fam.lead),
