    ...
    faqs: [
      {
        ...
        a: 'The USPTO sets its own filing fee per class, currently $350, paid directly to them. Our $299 is the professional fee for doing the work.',
      },
      {
...
const trademarkPage = {
  ...
  metaDescription:
    'Clearance search, USPTO filing and examiner responses handled by our legal desk. $299 per class plus the government filing fee.',
...
const addOnPages = addOns.items.map((item) => {
  const priceLabel = `${item.from ? 'from ' : ''}${money(item.price)}${item.per ? ` ${item.per}` : ''}`
  ...
  return {
    ...
    sections: [
      ...
      ...(item.per
        ? [
            {
              h: 'How the billing works',
              p: [
                `This one is ${money(item.price)} ${item.per}, not a one-off. It can be cancelled whenever you like — there is no minimum term, and nothing about your files or your site is held hostage if you stop.`,
              ],
            },
          ]
        : []),
