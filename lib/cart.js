...
export function cartLine(item, qty = 1) {
  return {
    ...
    /** Care plans and retainers are monthly, not one-off. The total has to be
     *  able to say which is which or the number is meaningless. */
    per: item.per || null,
    href: item.href || null,
    note: item.note || null,
 ...
 *
 * Recurring lines are totalled separately from one-off ones. Adding a $149 a
 * month care plan to a $2,999 one-off build and printing $3,148 would be
 * plainly wrong, so the two never share a number.
 */
export function cartTotals(items) {
  const oneOff = items.filter((i) => !i.per)
  const recurring = items.filter((i) => i.per)

  const sum = (list) => round2(list.reduce((n, i) => n + i.price * i.qty, 0))

  const subtotal = sum(oneOff)
  const tax = round2(subtotal * (salesTax.rate / 100))
  const recurringSubtotal = sum(recurring)
  const recurringTax = round2(recurringSubtotal * (salesTax.rate / 100))
  ...
  return {
    ...
    recurringSubtotal,
    recurringTax,
    recurringTotal: round2(recurringSubtotal + recurringTax),
...
export function orderSummaryText(items, totals, details = {}) {
  ...
  items.forEach((item, i) => {
    const unit = `${item.from ? 'from ' : ''}${cash(item.price)}${item.per ? ` ${item.per}` : ''}`
    lines.push(
  ...
  if (totals.hasRecurring) {
    lines.push('')
    lines.push(
      `Plus monthly items: ${cash(totals.recurringSubtotal)} + ${cash(
        totals.recurringTax
      )} tax = ${cash(totals.recurringTotal)} a month`
    )
  }
